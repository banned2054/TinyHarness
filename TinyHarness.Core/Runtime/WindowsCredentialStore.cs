...
public sealed partial class WindowsCredentialStore : ICredentialStore
{
        ...
        try
        {
            ...
            if (!CredWrite(ref credential, 0))
            {
                throw new InvalidOperationException(
                                                    $"Windows Credential Manager refused to save credential '{targetName}' (Win32 error {Marshal.GetLastWin32Error()}).");
            }
        }
        finally
        {
    ...
    public string? Read(string targetName)
    {
        ...
        if (!CredRead(targetName, GenericCredentialType, 0, out var credentialPtr))
        {
            ...
            return error == 1168 ? null : throw new InvalidOperationException(
                $"Windows Credential Manager failed to read credential '{targetName}' (Win32 error {error}).");
        }
    ...
    public bool Delete(string targetName)
    {
        ...
        if (!CredDelete(targetName, GenericCredentialType, 0))
        {
            ...
            throw new InvalidOperationException(
                                                $"Windows Credential Manager failed to delete credential '{targetName}' (Win32 error {error}).");
        }
