...
{
    public CliUsageException(string message, string? usage = null) : base(message)
    {
        Usage = usage;
    }
...
internal static class CommandLine
{
    ...
    private static CliOptions ParseConfigArgs(string[] args)
    {
        ...
        switch (subcommand)
        {
            ...
            case "set" when args.Length == 3 :
                ...
                {
                    Kind = CliCommandKind.Config, Subcommand = "set", Primary = args[1], Secondary = args[2],
                };
            case "set" :
    ...
    private static CliOptions ParseAuthArgs(string[] args)
    {
        ...
        return new CliOptions
        {
            Kind                = CliCommandKind.Auth, Subcommand = "set", Primary = profile,
            EnvironmentVariable = envVar, UseCredentialStore      = useStore,
        };
    }
    ...
    private static CliOptions ParseModelArgs(string[] args)
    {
        ...
        if (subcommand == "add")
        {
            ...
            return new CliOptions
            {
                Kind                = CliCommandKind.Model, Subcommand = "add", Primary = modelId,
                ContextWindowTokens = contextWindow, ProviderName      = provider,
            };
        }
        ...
        if (subcommand == "use")
        {
            ...
            return new CliOptions
            {
                Kind = CliCommandKind.Model, Subcommand = "use", Primary = args[1], ProviderName = provider,
            };
        }
    ...
    private static CliOptions ParseDoctorArgs(string[] args)
    {
        ...
        for (var i = 0; i < args.Length; i++)
        {
            ...
            if (string.Equals(args[i], "--connect", StringComparison.Ordinal))
            {
                connect = true;
                continue;
            }

            throw new CliUsageException($"Unknown argument '{args[i]}' for doctor.", HelpText.Doctor);
        }
