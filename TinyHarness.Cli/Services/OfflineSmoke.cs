...
/// <see cref="TinyHarness.Core.Models.Agent.AgentStatus.Failed"/> and makes the smoke exit non-zero.
/// </summary>
internal sealed class SmokeScriptClient(IReadOnlyList<SmokeStep> steps) : IChatCompletionClient
{
                                                                 ...
                                                                 CancellationToken cancellationToken)
    {
        ...
        if (turn > 0)
        {
            ...
            {
                throw new
                    InvalidOperationException($"Smoke: model request #{turn + 1} has no tool result in history; the Agent loop " +
...
internal static class SmokeScript
{
    ...
    /// </summary>
    private static SmokeStep Tool(string name, string argumentsJson, Action<string> verify) => new()
    {
        ToolName      = name,
