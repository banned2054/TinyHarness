        ...
        CommandContext context, string? providerName, CancellationToken cancellationToken)
    {
        ...
        if (!exists || config.Profiles.Count == 0)
        {
            throw new InvalidOperationException(
                                                "No provider profiles are configured yet. Run 'tinyharness init' or 'tinyharness provider add <name>'.");
        }
        ...
        if (!string.IsNullOrWhiteSpace(providerName))
        {
            ...
            {
                throw new InvalidOperationException(
                                                    $"Profile '{providerName}' was not found. Known profiles: {string.Join(", ", config.Profiles.Select(p => p.Name))}.");
            }
        ...
        {
            throw new InvalidOperationException(
                                                "No default profile is selected. Run 'tinyharness provider use <name>' or pass --provider <name>.");
        }
